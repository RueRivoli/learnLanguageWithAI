...
<template>
  <dialog :id="props.id" class="modal">
    ...
    />
      ...
      >
        <LayoutKeyElementRuleOption class="h-full cursor-pointer" v-for="(rule, n) in filteredModuleOptions" :key="n" :selected="moduleSelection[rule.id]" :title="rule.name" :titleEn="rule.nameEn" :symbol="rule.symbol" :level="rule.level" @click="handleRuleSelection(rule.id)" :lightMode="true">
                      <template #content>
                  <!-- Professional description box -->
                  <div v-if="rule.highlights" class="mt-3 mb-4">
                    <div class="relative rounded-xl p-4 shadow-sm overflow-hidden" :class="getBorderStyleClassFromGrammarRuleLevel(rule.level ?? 0)">
                      ...
                      <div class="relative z-10">
                        <div class="flex items-center gap-2 mb-2">
                          ...
                          <svg class="h-3 w-3" :class="getTextStyleClassFromGrammarRuleLevel(rule.level ?? 0)" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none"/>
                            <circle cx="12" cy="12" r="3" fill="currentColor"/>
                          </svg>
                          <span class="text-xs font-semibold uppercase tracking-wide" :class="getTextStyleClassFromGrammarRuleLevel(rule.level ?? 0)">Key Point</span>
                        </div>
