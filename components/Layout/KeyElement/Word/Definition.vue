...
<template>
  ...
  <div
    :class="[
      'text-slate-800 border-l-4 border-primary rounded-lg cursor-pointer transition-all duration-300',
      { 'bg-primary/10': props.isDarkMode },
  ...
  >
    ...
    >
      <div class="flex items-center gap-2">
        ...
        <span
          class="text-md text-neutral mr-1"
          :class="props.minified ? 'italic' : ''"
        >
          ({{ roles }})</span
        >
        <!-- text-slate-600 -->
        <span
          v-if="!props.minified"
          class="text-lg text-neutral font-light italic"
        >
    ...
    <div
      v-if="props.isActive && (props.word.sentence || props.word.sentenceEn)"
    ...
    >
      ...
      <div v-if="props.word.role2" class="flex items-center gap-2">
        <span class="text-md font-serif"> ({{ props.word.role }}) </span>
        <span class="text-md font-light italic">
          {{ props.word.translation }}
        </span>
      </div>
      <div v-if="props.word.sentence" class="text-md rounded-lg">
        <span>{{ props.word.sentence }}</span>
      </div>
      <div v-if="props.word.sentenceEn">
        <span>{{ props.word.sentenceEn }}</span>
      </div>
    </div>
    <div
      v-if="props.isActive && (props.word.sentence2 || props.word.sentence2En)"
      class="mt-3 pt-3 text-neutral border-t border-primary-50 space-y-2 animate-fade-in"
    >
      <div class="flex items-center gap-2">
        <span class="text-md font-serif"> ({{ props.word.role2 }}) </span>
        <span class="text-md font-light italic">
          {{ props.word.translation2 }}
        </span>
      </div>
      <div v-if="props.word.sentence2" class="text-md rounded-lg">
        <span>{{ props.word.sentence2 }}</span>
      </div>
      <div v-if="props.word.sentence2En">
        <span>{{ props.word.sentence2En }}</span>
      </div>
    </div>
    <div
      v-if="props.isActive && (props.word.sentence3 || props.word.sentence3En)"
      class="mt-3 pt-3 text-neutral border-t border-primary-50 space-y-2 animate-fade-in"
    >
      <div class="flex items-center gap-2">
        <span class="text-md font-serif"> ({{ props.word.role3 }}) </span>
        <span class="text-md font-light italic">
