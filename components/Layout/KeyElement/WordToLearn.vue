...
<template>
  ...
  >
    ...
    <div
      v-if="
        props.activeWordTranslation === props.index &&
        (props.word?.sentence || props.word?.sentenceEn)
      "
    ...
    >
      ...
      >
        <span class="font-medium text-slate-800">Example:</span>
        {{ props.word.sentence }}
      </div>
      <div v-if="props.word.sentenceEn" class="text-sm text-slate-600 pl-3">
        <span class="font-medium text-slate-800">Translation:</span>
        <span class="italic">{{ props.word.sentenceEn }}</span>
      </div>
    </div>
  </div>
</template>
