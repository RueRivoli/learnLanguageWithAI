<template>
  <div class="bg-white rounded-lg border border-gray-200 p-6">
    <div class="flex items-center gap-2 mb-4">
      ...
      <h3 class="text-lg font-semibold text-gray-900">Générer une image illustrative</h3>
    </div>
    ...
    <div v-if="result" class="mt-6">
      <div v-if="result.success && result.image_url" class="space-y-3">
        <div class="border rounded-lg overflow-hidden bg-gray-50">
          <img 
            ...
            :alt="lastPrompt"
            class="w-full h-auto"
            @load="$emit('imageGenerated', { url: result.image_url, prompt: lastPrompt })"
          />
        </div>
      ...
      <div v-else-if="!result.success" class="p-4 bg-red-50 border border-red-200 rounded-md">
...
const handleGenerate = async () => {
  ...
  if (props.context && !prompt.value.toLowerCase().includes(props.context.toLowerCase())) {
...
watch(() => props.suggestedPrompt, (newPrompt) => {
