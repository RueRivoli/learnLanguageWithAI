...
const getUnsupportedLanguages = async () => {
  try {
    ...
    const { data: languages } = await $fetch(
      "/api/languages/?is_supported=false",
    );
...
<template>
  ...
  <div id="unsupported-languages" class="mt-24">
    ...
    <div class="text-center mb-12">
      ...
      <h2
        class="text-3xl sm:text-4xl font-bold tracking-tight text-neutral mb-4"
      >
        Vote for <span class="text-warning">Your Language</span>
      </h2>
      <p
        class="text-lg sm:text-xl text-base-content/70 max-w-2xl mx-auto leading-relaxed"
      >
        Help us prioritize which languages to add next. Vote for the languages
        you'd like to learn!
      </p>
    </div>
      ...
      >
        ...
        >
          ...
          <div class="absolute bottom-2 right-2">
            <div
              class="inline-flex items-center gap-1.5 text-xs font-medium text-warning bg-warning/10 px-2.5 py-1 rounded-lg ring-1 ring-warning/20"
            >
              <HandThumbUpIcon
              ...
              <span class="text-neutral">{{ language.totalVotes }}</span>
            </div>
          </div>
          <div class="space-y-1">
            <h3 class="text-base font-semibold text-neutral pr-6">
              {{ language.name }}
            </h3>
            <p
              v-if="hasVoted(language.id)"
            ...
            <p v-else class="text-xs text-base-content/60">Click to vote</p>
          </div>
        </button>
        <!-- Skeleton Loaders -->
        <template v-else>
    ...
    <div v-if="languagesVoted.length > 0" class="mt-8 text-center">
      <div
        class="inline-flex flex-col items-center gap-3 rounded-lg bg-primary/10 px-6 py-4 ring-1 ring-primary/20"
      >
        <div class="flex items-center gap-2">
          <CheckCircleIcon class="h-5 w-5 text-primary" />
          <span class="font-semibold text-neutral">
            You voted for {{ languagesVoted.length }}
            {{ languagesVoted.length === 1 ? "language" : "languages" }}
          </span>
        </div>
  ...
  <div class="mt-16 mb-8 text-center">
    <div
      class="inline-flex flex-col items-center gap-4 rounded-lg bg-base-200/50 px-8 py-6 ring-1 ring-base-content/10"
    >
      <div class="flex items-center gap-2 text-base-content/70">
        <GlobeAltIcon class="h-5 w-5 text-primary" />
        <span class="font-semibold text-neutral"
          >More languages coming soon</span
        >
      </div>
      <p class="text-sm text-base-content/60 max-w-md">
        We're constantly expanding our language offerings based on your
        feedback!
      </p>
    </div>
  </div>
