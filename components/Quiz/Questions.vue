...
const isGrammarQuiz = computed(() => {
  return currentQuestionIndex.value < (props.grammarQuizQuestions?.length || 0);
});

const isWordsQuiz = computed(() => {
  return currentQuestionIndex.value >= (props.grammarQuizQuestions?.length || 0) && currentQuestionIndex.value < (props.grammarQuizQuestions?.length || 0) + (props.wordsForQuiz?.length || 0);
});

const isExpressionsQuiz = computed(() => {
  return currentQuestionIndex.value >= (props.grammarQuizQuestions?.length || 0) + (props.wordsForQuiz?.length || 0) && currentQuestionIndex.value < (props.grammarQuizQuestions?.length || 0) + (props.wordsForQuiz?.length || 0) + (props.expressionsForQuiz?.length || 0);
});
...
const currentQuestion = computed(() => {
  ...
  const grammarLength = props.grammarQuizQuestions?.length || 0;
  const wordsLength = props.wordsForQuiz?.length || 0;
  if (currentQuestionIndex.value < grammarLength) {
     // Grammar sectionn    
  return props.grammarQuizQuestions?.[currentQuestionIndex.value] || null;
} else if (currentQuestionIndex.value < grammarLength + wordsLength) {
    // Words section
    const wordIndex = currentQuestionIndex.value - grammarLength;
    return props.wordsQuizQuestions?.[wordIndex] || null;
  } else {
    // Expressions section
    const expressionIndex = currentQuestionIndex.value - grammarLength - wordsLength;
    return props.expressionsQuizQuestions?.[expressionIndex] || null;
  }
});
...
const currentSection = computed(() => {
  ...
  if (isGrammarQuiz.value) {
    ...
    const wordsCount = props.wordsForQuiz?.length || 0;
    const expressionsCount = props.expressionsForQuiz?.length || 0;
    const grammarCount = props.grammarQuizQuestions?.length || 0;
    
    if (currentQuestionIndex.value < (props.grammarQuizQuestions?.length || 0) + wordsCount) {
      return {
        name: 'Words',
...
const totalQuestions = computed(() => {
  return (props.grammarQuizQuestions?.length || 0) + 
         (props.wordsForQuiz?.length || 0) + 
         (props.expressionsForQuiz?.length || 0);
});
...
const grammarScore = computed(() => {
  if (!props.grammarQuizQuestions || !formGrammarQuiz.value) return 0;
  let correct = 0;
  let total = 0;
...
const wordsScore = computed(() => {
  if (!props.wordsQuizQuestions || !formWordsQuiz.value) return 0;
  let correct = 0;
  let total = 0;
...
const expressionsScore = computed(() => {
  if (!props.expressionsQuizQuestions || !formExpressionsQuiz.value) return 0;
  let correct = 0;
  let total = 0;
