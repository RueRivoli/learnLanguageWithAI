...
export const useQuiz = (
  grammarQuizQuestions: Ref<GrammarQuizQuestion[] | null>,
  wordsQuizQuestions: Ref<VocabularyQuizQuestion[] | null>,
  expressionsQuizQuestions: Ref<VocabularyQuizQuestion[] | null>,
  wordsForQuiz: Ref<WordContent[] | null>,
  expressionsForQuiz: Ref<ExpressionContent[] | null>,
  type: "full" | "vocabulary" | "grammar",
  emit: (event: "submitQuiz", payload: any) => void,
) => {
  ...
  watch(
    () => grammarQuizQuestions.value,
    (newGrammarQuizQuestions: GrammarQuizQuestion[] | null) => {
      if (newGrammarQuizQuestions)
        initializeFormQuiz(formGrammarQuiz, newGrammarQuizQuestions);
    },
    { immediate: true },
  );

  watch(
    () => wordsQuizQuestions.value,
    (newWordsForQuizQuestions: VocabularyQuizQuestion[] | null) => {
      if (newWordsForQuizQuestions)
        initializeFormQuiz(formWordsQuiz, newWordsForQuizQuestions);
    },
    { immediate: true },
  );

  watch(
    () => expressionsQuizQuestions.value,
    (newExpressionsForQuizQuestions: VocabularyQuizQuestion[] | null) => {
      if (newExpressionsForQuizQuestions)
        initializeFormQuiz(formExpressionsQuiz, newExpressionsForQuizQuestions);
    },
    { immediate: true },
  );
  ...
  const grammarSectionQuizLength = computed(() => {
    return type === "grammar"
      ? TOTAL_GRAMMAR_QUESTIONS_GRAMMAR_QUIZ
      : TOTAL_GRAMMAR_QUESTIONS_FULL_QUIZ;
  });

  const isGrammarQuiz = computed(() => {
    return (
      currentQuestionIndex.value < (grammarQuizQuestions.value?.length || 0)
    );
  });
  ...
  const isWordsQuiz = computed(() => {
    return (
      ...
      currentQuestionIndex.value <
        (grammarQuizQuestions.value?.length || 0) +
  ...
  const currentQuestion = computed(
    (): GrammarQuizQuestion | VocabularyQuizQuestion | null => {
      ...
      if (currentQuestionIndex.value < grammarLength) {
        ...
        const expressionIndex =
          currentQuestionIndex.value - grammarLength - wordsLength;
  ...
  const currentSection = computed(() => {
    if (isGrammarQuiz.value) {
      ...
      if (
        currentQuestionIndex.value <
        (grammarQuizQuestions.value?.length || 0) + wordsCount
      ) {
        return {
          name: "Words",
  ...
  const currentQuestionOptions = computed(
    (): Array<
      GrammarQuizQuestion["option1"] | VocabularyQuizQuestion["option1"]
    > => {
  ...
  const globalScore = computed(() => {
    const grammarTotal = Object.values(formGrammarQuiz.value).filter(
      (a) => a.selectedOption !== null,
    ).length;
    const wordsTotal = Object.values(formWordsQuiz.value).filter(
      (a) => a.selectedOption !== null,
    ).length;
    const expressionsTotal = Object.values(formExpressionsQuiz.value).filter(
      (a) => a.selectedOption !== null,
    ).length;
    ...
    const totalCorrect =
      Object.values(formGrammarQuiz.value).filter(
        (a) =>
          a.selectedOption !== null &&
          Number(a.selectedOption) === Number(a.correctAnswer),
      ).length +
      Object.values(formWordsQuiz.value).filter(
        (a) =>
          a.selectedOption !== null &&
          Number(a.selectedOption) === Number(a.correctAnswer),
      ).length +
      Object.values(formExpressionsQuiz.value).filter(
        (a) =>
          a.selectedOption !== null &&
          Number(a.selectedOption) === Number(a.correctAnswer),
      ).length;
  ...
  const wordsProgress = computed((): QuizProgress[] => {
    ...
    for (let i = 0; i < wordsForQuiz.value?.length; i++) {
      ...
      if (isVocabularyQuiz.value) {
        const hasAnswer =
          formWordsQuiz.value[i + 1] &&
          formWordsQuiz.value[i + 1].selectedOption !== null;
        progress.push({
          completed:
  ...
  const expressionsProgress = computed((): QuizProgress[] => {
    ...
    for (let i = 0; i < expressionsForQuiz.value?.length; i++) {
      ...
      if (isExpressionsQuiz.value) {
        const hasAnswer =
          formExpressionsQuiz.value[i + 1] &&
          formExpressionsQuiz.value[i + 1].selectedOption !== null;
        progress.push({
          completed:
            !isQuizCompleted.value &&
            questionIndex < currentQuestionIndex.value,
  ...
  const isQuestionCorrect = (questionIndex: number) => {
    ...
    if (questionIndex < grammarLength) {
      ...
      return (
        ...
        Number(answer.selectedOption) === Number(answer.correctAnswer)
      );
    } else if (questionIndex < grammarLength + wordsLength) {
      const formIndex = questionIndex - grammarLength + 1;
      const answer = formWordsQuiz.value[formIndex];
  ...
  const goToNextQuestion = () => {
    if (selectedAnswer.value) {
      ...
      const optionIndex = currentQuestionOptions.value.findIndex(
        (option) => option === selectedAnswer.value,
      );
      ...
      if (
        isGrammarQuiz.value &&
        formGrammarQuiz.value[currentQuestionIndex.value + 1]
      ) {
        formGrammarQuiz.value[currentQuestionIndex.value + 1].selectedOption =
        ...
        isExpressionsQuiz.value &&
        formExpressionsQuiz.value[
          currentQuestionIndex.value -
            (grammarQuizQuestions.value?.length || 0) -
            (wordsForQuiz.value?.length || 0) +
            1
  ...
  const detailedResults = computed((): DetailedResults => {
    const grammarCorrect = Object.values(formGrammarQuiz.value).filter(
      (a) =>
        a.selectedOption !== null &&
        Number(a.selectedOption) === Number(a.correctAnswer),
    ).length;
    ...
    const validatedWordsList: Array<{
      text: string;
      isMastered: boolean;
      id: number;
    }> = [];
    const invalidatedWordsList: Array<{
      text: string;
      isMastered: boolean;
      id: number;
    }> = [];
    if (type !== "grammar" && wordsQuizQuestions.value && formWordsQuiz.value) {
      Object.values(formWordsQuiz.value).forEach((answer, index) => {
        if (
          answer.selectedOption !== null &&
          wordsQuizQuestions.value?.[index]
        ) {
          const question = wordsQuizQuestions.value?.[index];
          const correctAnswer: VocabularyQuizQuestion["option1"] =
            question[`option${answer.correctAnswer}`];
          const userAnswer = question[`option${answer.selectedOption}`];
          const wordContext = wordsForQuiz.value?.find(
            (word) => word.text === correctAnswer,
          );
          if (Number(answer.selectedOption) === Number(answer.correctAnswer)) {
            validatedWordsList.push({
              text: correctAnswer,
              isMastered: wordContext?.isMastered ?? false,
              id: wordContext?.id ?? 0,
            });
          } else {
            invalidatedWordsList.push({
              text: correctAnswer,
              isMastered: wordContext?.isMastered ?? false,
              id: wordContext?.id ?? 0,
            });
    ...
    const validatedExpressionsList: Array<{
      text: string;
      isMastered: boolean;
      id: number;
    }> = [];
    const invalidatedExpressionsList: Array<{
      text: string;
      isMastered: boolean;
      id: number;
    }> = [];
    if (
      ...
      Object.values(formExpressionsQuiz.value).forEach((answer, index) => {
        if (
          answer.selectedOption !== null &&
          expressionsQuizQuestions.value?.[index]
        ) {
          const question = expressionsQuizQuestions.value?.[index];
          const correctAnswer = question[`option${answer.correctAnswer}`];
          const userAnswer = question[`option${answer.selectedOption}`];
          const expressionContext = expressionsForQuiz.value?.find(
            (expr) => expr.text === correctAnswer,
          );
          if (Number(answer.selectedOption) === Number(answer.correctAnswer)) {
            validatedExpressionsList.push({
              text: correctAnswer,
              isMastered: expressionContext?.isMastered ?? false,
              id: expressionContext?.id ?? 0,
            });
          } else {
            invalidatedExpressionsList.push({
              text: correctAnswer,
              isMastered: expressionContext?.isMastered ?? false,
              id: expressionContext?.id ?? 0,
            });
    ...
    return type !== "grammar"
      ? {
          ...
          words: {
            ...
            percentage:
              wordsTotal > 0
                ? Math.round((wordsCorrect / wordsTotal) * 100)
                : 0,
          ...
          expressions: {
            ...
            percentage:
              expressionsTotal > 0
                ? Math.round((expressionsCorrect / expressionsTotal) * 100)
                : 0,
