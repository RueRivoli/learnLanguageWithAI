...
export default defineEventHandler(async (event) => {
  ...
  if (query.is_learned === "true") {
    request = supabase
      ...
      .select(
        "id, text, translation, expression_sentence, expression_sentence_translation, expression_sentence_2, expression_sentence_2_translation, turkish_expressions_knowledge!inner(expression_mastered)",
        { count: "exact" },
      )
    ...
    const { data } = await supabase
      .from("turkish_expressions")
      .select("id, turkish_expressions_knowledge!inner(expression_mastered)")
      .eq("turkish_expressions_knowledge.expression_mastered", true);
    if (data) {
