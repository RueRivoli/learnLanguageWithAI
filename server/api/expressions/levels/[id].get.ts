...
export default defineEventHandler(async (event) => {
  ...
  const knownExpressionIdsToExclude = knownExpressions.map(
    (w) => w.expression_id,
  );
  let req = supabase
  ...
  const { data: allUnknownExpressions, error: unknownExpressionsError } =
    await req.limit(limitItems * 3); // Get 3x more expressions to have a good pool for randomization
  ...
  const unknownExpressions = shuffleArray(allUnknownExpressions || []).slice(
    0,
    limitItems,
  );
