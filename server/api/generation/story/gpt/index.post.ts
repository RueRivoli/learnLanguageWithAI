...
const linkExpressionsToLesson = async (
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const {
      data: { user },
      error: userError,
    } = await supabaseAuthToken.auth.getUser();
    ...
    if (!profile || (profile.credits_available || 0) < 1) {
      throw new Error(
        "Insufficient credits. Please purchase more credits to generate stories.",
      );
    }
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a native experimented turkish teacher. Create a clear, captivating and helpful story to learn Turkish",
          },
    ...
    if (json && json.choices?.[0]?.message?.content) {
      const lesson = await saveNewLesson(
