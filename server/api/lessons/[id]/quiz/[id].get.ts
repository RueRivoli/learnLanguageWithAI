...
} from "~/server/utils/auth/supabaseClient";
...
export default defineEventHandler(async (event) => {
  ...
  .eq("quiz_id", quizId)
    ...
    const { data : wordQuizzes, error : wordQuizzesError } = await supabase
      ...
      .eq("quiz_id", quizId)
    const { data : expressionQuizzes, error : expressionQuizzesError } = await supabase
      ...
      .eq("quiz_id", quizId)
  if (grammarQuizzesError || wordQuizzesError || expressionQuizzesError) throw grammarQuizzesError || wordQuizzesError || expressionQuizzesError;
