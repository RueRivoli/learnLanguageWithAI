...
const getVocabularyFromLesson = async (event: any, lessonId: number) => {
    ...
    const { data, error } = await supabase.from("turkish_lessons")
      .select(
    ...
    const wordsForQuiz : Array<{id: number, isMastered: boolean}> = (data?.turkish_lesson_words || []).map((word: any) => {
      return { ...word.turkish_words, isMastered: false };
    });
    const expressionsForQuiz : Array<{id: number, isMastered: boolean}> = (data?.turkish_lesson_expressions || []).map((expression: any) => {
    ...
    const { data: randomExpressions } = await $fetch(`/api/expressions/levels/random/?limit=2`, {
      method: "GET",
      headers,
    });
    wordsForQuiz.push(...randomWords.map((word: any) => {
        return { ...{id: word.turkish_words.id}, isMastered: true };
    }));
    expressionsForQuiz.push(...randomExpressions.map((expression: any) => {
...
const createWordQuiz = async (event: any, wordsForQuiz: Array<{id: number, isMastered: boolean}>, quizId: number) => {
    const supabase = createSupabaseClientWithUserAuthTokenFromHeader(event);
    let wordQuizzes: any[] = [];
...
const createExpressionQuiz = async (event: any, expressionsForQuiz: Array<{id: number, isMastered: boolean}>, quizId: number) => {
    const supabase = createSupabaseClientWithUserAuthTokenFromHeader(event);
    let expressionQuizzes = [];
    const expressionIds = expressionsForQuiz.map((exp) => exp.id);
    if (expressionsForQuiz.length > 0) {
      const { data: expressionQuizRows, error: expressionQuizError } = await supabase
...
const createVocabularyQuiz = async (event: any, lessonId: number, quizId: number) => {
  const { wordsForQuiz, expressionsForQuiz } = await getVocabularyFromLesson(event, lessonId);
  const wordQuizzes = await createWordQuiz(event, wordsForQuiz, quizId);
    const expressionQuizzes = await createExpressionQuiz(event, expressionsForQuiz, quizId);
