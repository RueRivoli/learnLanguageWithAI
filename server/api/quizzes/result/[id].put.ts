...
const updateVocabularyCountScores = async (supabase: any, userId: string) => {
  const countWordsMastered = await supabase
    .from("turkish_words_knowledge")
    .select("*", { count: "exact", head: true })
    .eq("user_id", userId)
    .eq("word_mastered", true);
  const countExpressionsMastered = await supabase
    .from("turkish_expressions_knowledge")
    .select("*", { count: "exact", head: true })
    .eq("user_id", userId)
    .eq("expression_mastered", true);
  const { error: errorFromVocabularyScores } = await supabase
    .from("turkish_vocabulary_scores")
    .update({
      words_mastered_count: countWordsMastered.count,
      expressions_mastered_count: countExpressionsMastered.count,
    })
    .eq("user_id", userId)
    .eq("language", "tr");
  if (errorFromVocabularyScores)
    throw "An error occured while updating the vocabulary scores, please try again";
};

const updateVocabularyKnowledge = async (
  supabase: any,
  userId: string,
  newMasteredWordsIds: number[],
  newForgottenWordIds: number[],
  newMasteredExpressionsIds: number[],
  newForgottenExpressionsIds: number[],
) => {
  // New Words have been learned
  const { error: errorFromWordsKnowledge } = await supabase
    .from("turkish_words_knowledge")
    .upsert(
      newMasteredWordsIds.map((wordId) => ({
        user_id: userId,
        word_id: wordId,
        word_learned: true,
        word_mastered: true,
  ...
  if (errorFromWordsKnowledge)
    throw "An error occured while updating the words knowledge, please try again";
  ...
  if (errorFromForgottenWordsKnowledge)
    throw "An error occured while updating the forgotten words knowledge, please try again";

  // New Expressions have been learned
  const { error: errorFromExpressionsKnowledge } = await supabase
    .from("turkish_expressions_knowledge")
    .upsert(
      newMasteredExpressionsIds.map((exprId) => ({
        user_id: userId,
        expression_id: exprId,
        expression_learned: true,
        expression_mastered: true,
      })),
    );
  if (errorFromExpressionsKnowledge)
    throw "An error occured while updating the expressions knowledge, please try again";
  ...
  if (errorFromForgottenExpressionsKnowledge)
    throw "An error occured while updating the forgotten expressions knowledge, please try again";
  ...
  try {
    ...
    const updateQuizResult = async () => {
      ...
      if (errorFromQuizzedResult) {
        throw new Error(
          "An error occurred while updating the quiz result. Please try again.",
        );
      }
    };
    ...
    const updateUserGrammarScore = async () => {
      const { error: errorFromGrammarScores } = await supabase
        ...
        .eq("rule_id", ruleId)
        .eq("user_id", userId);
      if (errorFromGrammarScores)
        throw "An error occured while updating the quiz result, please try again";
    };
    ...
    const updateVocabularyData = async () => {
      if (userId)
        updateVocabularyKnowledge(
          ...
          newMasteredWordsIds,
          newForgottenWordIds,
          newMasteredExpressionsIds,
          newForgottenExpressionsIds,
        );
    };
    // rememberQuizSelection()
    await Promise.all([
      updateQuizResult(),
      updateUserGrammarScore(),
