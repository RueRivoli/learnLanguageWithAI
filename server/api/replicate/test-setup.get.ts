...
export default defineEventHandler(async (event) => {
  try {
    ...
    const bucketExists = buckets.some(
      (bucket) => bucket.name === envCheck.SUPABASE_BUCKET_NAME,
    );
    const bucketIsPublic = buckets.find(
      (bucket) => bucket.name === envCheck.SUPABASE_BUCKET_NAME,
    )?.public;
    ...
    return {
      ...
      recommendations: [
        ...(!envCheck.SUPABASE_URL
          ...
          : []),
        ...(!envCheck.SERVICE_SUPABASE_ANON_KEY
          ? ["Set SERVICE_SUPABASE_ANON_KEY environment variable"]
          : []),
        ...
        ...(!supabaseConnection
          ...
          : []),
        ...(!bucketExists
          ? [`Create '${envCheck.SUPABASE_BUCKET_NAME}' bucket in Supabase`]
          : []),
        ...(bucketExists && !bucketIsPublic
          ? [`Make '${envCheck.SUPABASE_BUCKET_NAME}' bucket public`]
          : []),
      ],
    };
  } catch (error) {
