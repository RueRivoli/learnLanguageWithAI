...
export default defineEventHandler(async (event) => {
  ...
  let req = supabase.from("turkish_words").select("id, text, translation");
  ...
  const { data: allUnknownWords, error: unknownWordsError } = await req.limit(
    limitItems * 3,
  ); // Get 3x more words to have a good pool for randomization
